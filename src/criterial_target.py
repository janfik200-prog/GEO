...
def load_real_points(meta: integro_grid.GridMeta) -> np.ndarray:
    """Плоские индексы ячеек сетки prognoz с точками внешней заверки.

    ВАЖНО (установлено 30.07.2026): единственный точечный слой в переданных
    шейпах — «геохимическое_опробование» (76 точек -> 73 ячейки): 67
    аномальных литохимических проб (Au/U/...), 5 шлиховых проб Au и только
    4 собственно проявления (все U/TR, золотых нет). Это НЕ «реальные
    рудопроявления», а точки заверки; использовать их можно (постановка
    разрешает прямые признаки для заверки, не для обучения), но формулировать
    результаты как «аномальные пробы + проявления». Каveат: пробы в рыхлых
    отложениях отбираются вдоль гидросети, а ``dist_dnara`` входит в топ
    признаков модели — часть захвата может быть геометрией опробования.
    Точки не участвуют нигде в конвейере (ни в критериальном анализе, ни
    в обучении на нём).
    """
...
def real_point_verification(
    ...
    """Пункт 5: ML (критериальный таргет) против baseline критериального на точках заверки.
    ...
    на данных, которые критериальный анализ вообще не видел (точки заверки, см.
    :func:`load_real_points` — аномальные геохимические пробы + 4 проявления).

    ``coverage_ml`` использует усреднение по всем LOO-моделям. Осторожно: это НЕ
    out-of-fold скор — ячейки объекта k были обучающими позитивами в 2 из 3
    моделей, поэтому на точках внутри объектов lift_ml завышен запоминанием.
    Для честного сравнения строить OOF-скор (для ячеек объекта k — прогноз
    фолда, где k был контрольным) и/или считать дельту на фоновых точках.
    Baseline при этом in-sample по всей территории (LOO-версии у критериального
    нет) — сравнение структурно смещено в пользу baseline.
    """
