...
# --- Критериальный анализ как ТАРГЕТ (задача: цель = прогноз ГИС Интегро, не геохимия) ---
# Нативный результат критериального расчёта «Таксономия по критериям» (float32 LE,
# 22946 значений на сетке 154×149). Меньше = перспективнее (мера Плюты).
GOLD_TARGET_CRITERIAL: Path = (
    PROJECT_ROOT / "data" / "Gis-integro" / "Расчет" / "prognoz.prognoz.property"
)
# Датасет признаков dataset_v1 (геохимия исключена стоп-листом при сборке).
CRITERIAL_DATASET_PARQUET: Path = PROJECT_ROOT / "data" / "dataset_v1" / "dataset_v1.parquet"
# Доля наиболее перспективных ячеек в положительном классе (бинаризация таргета).
CRITERIAL_TOP_FRAC: float = 0.15
# Признаки, ВЫВЕДЕННЫЕ из тех же факторов, что и критериальный таргет — исключаются,
# чтобы модель не восстанавливала формулу по её же входам (утечка/тавтология).
# 6 dist_* критерия + плотности тех же тектонических/магматических факторов.
# Модель учится на НЕЗАВИСИМЫХ данных: гравика/магнитка, Landsat-7, рельеф,
# dist_dnl/dnara (не входят в критериальный), маска свиты.
# Бинарные геологические признаки (членство ячейки в объекте: разломы, фации,
# палеодолины, дайки, коры). Заменяют непрерывные dist_* факторы — грубее, но
# геологически осмысленны и не воспроизводят формулу критериального буквально.
# Строятся в src/geo_indicators.py из шейпов ГИС Интегро.
CRITERIAL_GEO_INDICATORS: bool = True
# Производные гео-признаки (mineral systems): пересечения факторов (И индикаторов),
# coincidence (число совпавших факторов), плотности разломов/даек из датасета.
CRITERIAL_GEO_DERIVED: bool = True
# Производные рельефа (уклон/кривизна/TPI/шероховатость) из relief_m —
# восполняют отсутствующие в dataset_v1 терренные признаки (в GEO их было 5).
CRITERIAL_TERRAIN_DERIVED: bool = True
CRITERIAL_TERRAIN_WINDOW: int = 5     # окно TPI/шероховатости, ячеек (~2.5 км при 500 м)

CRITERIAL_EXCLUDE_FEATURES: tuple[str, ...] = (
    "dist_facies", "dist_struct", "dist_magm", "dist_paleo",
    "dist_tect1", "dist_tect2", "dens_tect", "dens_magm",
)
# Сторона пространственного блока (в ячейках) для GroupKFold — против утечки
# через пространственную автокорреляцию соседних ячеек.
CRITERIAL_BLOCK_CELLS: int = 15
# Число фолдов и метрики lift@N% для валидации критериального классификатора.
CRITERIAL_N_SPLITS: int = 5
CRITERIAL_LIFT_AREAS: tuple[float, ...] = (0.05, 0.10, 0.15, 0.20)
CRITERIAL_RANDOM_STATE: int = 42
# Отдельные (более лёгкие) гиперпараметры леса для критериального классификатора —
# полная разметка (все ячейки) не требует 800 деревьев; ускоряет CV без потери качества.
CRITERIAL_RF_TREES: int = 300
CRITERIAL_GB_TREES: int = 150
