...
# Суффиксы служебных колонок ЛЮБОГО спутникового источника (этап 8: их стало
# пять). Признаком качества съёмки нельзя прогнозировать руду: карта числа
# наблюдений — это геометрия чужих орбит и тайлов, а не свойство площади.
SERVICE_SUFFIXES: tuple[str, ...] = ("_n_obs", "_valid_frac")


def is_service(col: str) -> bool:
    """Служебная ли колонка (по имени или по суффиксу источника)."""
    return col in SERVICE_COLS or col.endswith(SERVICE_SUFFIXES)
...
# Этап 8 добавил s1_/psr_/l8_: они тоже обязаны быть здесь, иначе 30% пропусков
# ASTER и дыры радара начали бы двигать саму маску, то есть площадь оценки, и
# lift этапа 9 стал бы несравним с этапами 1-7 не из-за метода, а из-за площади.
