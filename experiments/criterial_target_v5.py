"""Задача 3 на dataset_v5: leave-one-object-out на расширенном наборе признаков.

Первый прогон задачи (``src/criterial_target.py``, реестр задач п.3) использовал
29 независимых признаков датасета v1 (гравика/магнитка, Landsat 7, рельеф,
гидросеть). К моменту сборки датасета v5 (этап 9 и далее) добавились
Sentinel-1/2, Landsat 8/9, PALSAR-2, ASTER VNIR/SWIR/TIR, потенциальные поля
и их трансформанты, доп. рельеф — 167 независимых признаков вместо 29
(``src.criterial_target.training_features(dataset="v5")``, группы geo/geo2
исключены по той же причине циркулярности, что и в v1 — см. докстринг).

Протокол не меняется: те же 3 псевдо-объекта (209/97/79 ячеек, порог 0.15 на
prognoz), тот же leave-one-object-out с пространственным буфером, тот же
перестановочный тест и сид-свип. Меняется только признаковое пространство —
это прямая проверка, была ли неудача v1 нехваткой признаков или структурным
дефицитом независимых локаций (n=3 объекта).

Запуск из корня: ``python -X utf8 -m experiments.criterial_target_v5``.
"""
...
def main() -> None:
    ...
    buf_summary.to_csv(out_dir / "criterial_target_v5_buffer.csv", index=False)
    summary_df.to_csv(out_dir / "criterial_target_v5_loo.csv", index=False)
    perm_df.to_csv(out_dir / "criterial_target_v5_perm.csv", index=False)
    sweep_summary.to_csv(out_dir / "criterial_target_v5_sweep.csv", index=False)
    imp_df.to_csv(out_dir / "criterial_target_v5_importance.csv")
    print(f"\nсохранено: {out_dir}/criterial_target_v5_*.csv")
    ...
    fig.suptitle(f"Задача 3 на dataset_v5 ({len(feature_names)} независимых признаков): "
                f"leave-one-object-out, буфер {config.CRIT_HOLDOUT_BUFFER_M / 1000:.0f} км "
                "(красные точки — held-out объект)", fontsize=10)
    map_path = ROOT / "outputs" / "criterial_target_v5_map.png"
